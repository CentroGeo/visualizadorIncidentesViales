...
preprocesa_pgj <- function(pgj) {
  pgj["hora_de_hechos"] <- chron::times(paste0(substr(pgj$fecha_hechos, 12, 16), ":00"))
  pgj["fecha_de_hechos"] <- chron::dates(substr(pgj$fecha_hechos, 1, 10), format = "y-m-d")
  pgj["timestamp"] <- chron::chron(pgj$fecha_de_hechos, pgj$hora_de_hechos)
  pgj <- dplyr::filter(pgj, timestamp >= chron::dates("2016-01-01", format = "y-m-d"))
  pgj["geopoint"] <- NULL
  ...
  pgj <- sf::st_transform(sf::st_as_sf(pgj,
...
preprocesa_ssc <- function(path) {
  ...
  ssc$hora_evento <-
    times(paste0(
      ...
      ":",
      ifelse(
        str_sub(ssc$hora_evento, -1) == ".",
        substr(ssc$hora_evento, 4, 5),
        str_sub(ssc$hora_evento, -2)
      ),
      ":00"
    ))
  ...
  ssc$no_folio[ssc$no_folio == "SD"] <-
    paste0("SinID_", seq(1:nrow(dplyr::filter(
      ssc, no_folio == "SD"
    ))))
  tmp <-
    dplyr::filter(as.data.frame(table(ssc$no_folio), stringsAsFactors = FALSE), Freq > 1)
  for (i in tmp$Var1) {
  ...
  # =
  ssc <-
    dplyr::filter(ssc, !is.na(coordenada_y) & !is.na(coordenada_x))
